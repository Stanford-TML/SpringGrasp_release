...
class GPISUnified:
    def __init__(self, sigma=0.6, bias=2, kernel="tps", pcd=None, device="cpu"):
        self.sigma = sigma
        self.bias = bias
        self.fraction = None
        self.pcd = pcd
    ...
    def _preprocess_pcd(self):
        bound = max(max(self.pcd.get_axis_aligned_bounding_box().get_extent()) / 2 + 0.01, 0.1) # minimum bound is 0.1
        center = self.pcd.get_axis_aligned_bounding_box().get_center()
        pcd_simple = self.pcd.farthest_point_down_sample(200)
        points = np.asarray(pcd_simple.points)
        points = torch.tensor(points).to(self.device).double()
        data_noise = [0.005] * len(points)
        weights = torch.rand(50,len(points)).to(self.device).double()
        ...
        externel_points = torch.tensor([[-bound, -bound, -bound], 
                                        ...
                                        [0., bound, 0],
                                        [0., 0., bound], 
                                        [0., 0., -bound]]).double().to(self.device)
        externel_points += torch.from_numpy(center).to(self.device).double()
        y = torch.vstack([bound * torch.ones_like(externel_points[:,0]).to(self.device).view(-1,1),
                        torch.zeros_like(points[:,0]).to(self.device).view(-1,1),
                        -bound * 0.3 * torch.ones_like(internal_points[:,0]).to(self.device).view(-1,1)])
        self.fit(torch.vstack([externel_points, points, internal_points]), y,
                               noise = torch.tensor([0.2] * len(externel_points)+
                               data_noise +
                               [0.05] * len(internal_points)).double().to(self.device))
        self.bound = bound
        self.center = center
    ...
    def get_visualization_data(self, steps=100):
        ub = [self.bound + self.center[0], self.bound + self.center[1], self.bound + self.center[2]]
        lb = [-self.bound + self.center[0], -self.bound + self.center[1], -self.bound + self.center[2]]
        test_X = torch.stack(torch.meshgrid(torch.linspace(lb[0],ub[0],steps),
                                            torch.linspace(lb[1],ub[1],steps),
                                            torch.linspace(lb[2],ub[2],steps),indexing="xy"),dim=3).double().to(self.X1.device) # [steps, steps, steps, 3]
        test_mean, test_var = torch.zeros(steps,steps,steps), torch.zeros(steps,steps,steps)
    ...
    def topcd(self,test_mean, test_normal, test_var=None, steps=100):
        ub = [self.bound + self.center[0], self.bound + self.center[1], self.bound + self.center[2]]
        lb = [-self.bound + self.center[0], -self.bound + self.center[1], -self.bound + self.center[2]]
        lb, ub = np.asarray(lb), np.asarray(ub)
